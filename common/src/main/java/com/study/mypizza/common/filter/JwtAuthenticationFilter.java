...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // JWT 토큰 검증에서 제외할 url ( equalsIgnoreCase() 비교 )
...
//                    "/css/**", "/js/**", "/images/**"   // static content
                    ...
                    ,"/api/auth/logout"        // 회원가입 URL을 추가합니다.
                    ,"/api/auth/sign-up"        // 회원가입 URL을 추가합니다.
                    ,"/api/auth/sign-in"        // 로그인 액션 URL을 추가합니다.
                    ,"/common/layout"        // 로그인 액션 URL을 추가합니다.
                    ,"/common/top"        // 로그인 액션 URL을 추가합니다.
                    ,"/common/config"        // 로그인 액션 URL을 추가합니다.
            );

    // JWT 토큰 검증에서 제외할 url ( startsWith() 비교)
    ...
    // JWT 인증 커스텀 필터
    // 하는일!!
    // 1) 요청(Request) 헤더에서 JWT 토큰을 추출, 유효성 검증
    // 2) JWT 토큰에서 유저명(username)과 권한(authorities)을 얻어와 스프링 시큐리티용 인증 토큰(Authentication) 생성
    // 3) SecurityContext에 Authentication 등록
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            log.trace("### 잘못된 JWT 서명입니다. [{}]", e.getMessage());
    ...
    // 요청 헤더에서 JWT RefreshToken을 추출해서 리턴
    private String resolveRefreshToken(HttpServletRequest request) {
        // request에서 Authorization 헤더를 찾음
        String authorizationString = request.getHeader(TokenProvider.REFRESH_TOKEN_COOKIE) ;
...
//            if ( cookies != null ) {
//                for (Cookie cookie:cookies) {
//                    if (TokenProvider.xxx .equals(cookie.getName())) {
//                        log.trace("### HttpHeaders.AUTHORIZATION : {}", cookie.getValue());
//                        authorizationString = cookie.getValue() ;
//                        break;
//                    }
//                }
//            }
            ...
            if ( authorizationString == null ) {
                log.trace("### RefreshToken authorizationString is NULL!! Path : {}", request.getRequestURI());
            }
        }
