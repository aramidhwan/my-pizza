...
public class HikariPoolPreWarm {
    ...
    @PostConstruct
    public void warmUp() {
        System.out.println("🔧 HikariCP 커넥션 풀 Pre-warming 시작...");

        for (int i = 0; i < WARMUP_CONNECTIONS; i++) {
            try (Connection conn = dataSource.getConnection()) {
                // 바로 반환하면 풀에 커넥션이 채워짐
            } catch (SQLException e) {
                System.err.println("⚠️ 커넥션 풀 pre-warm 실패: " + e.getMessage());
            }
        }

        System.out.println("✅ HikariCP 커넥션 풀 Pre-warming 완료 (" + WARMUP_CONNECTIONS + "개)");
    }
}
